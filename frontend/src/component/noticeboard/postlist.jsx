    ...
    const getPost = async () => {
        try {
            ...
            if (searchType === "search") {
                // 검색 요청 처리
                getPostResult = await axios({
                    method: "GET",
                    url: `/posts/search/${searchData}?page=${pages - 1}&size=10`,
                });
            } else {
                // 일반 게시글 요청 처리
                getPostResult = await axios({
            ...
            console.log("게시글 데이터: ", getPostResult.data);
    
            // 번호 필드 수동 추가
            const postsWithNumber = getPostResult.data.map((post, index) => ({
                ...post,
                number: index + 1 + (pages - 1) * 10, // 페이지에 따른 번호 계산
            }));
            setPostAll(postsWithNumber);
    
            // 게시글 총 개수 가져오기
            const countUrl = searchType === "search"
                ? `/posts/count?type=search&data=${searchData}`
                : `/posts/count?type=normal`;
            ...
            console.log("게시글 개수: ", result.data);
            setTotalPost(result.data);
        } catch (error) {
            console.error("게시글 조회 중 오류 발생: ", error.response || error);
