...
const NoticeFrame = () => {
    ...
    const getInitData = async () => {
        try {
            ...
            localStorage.setItem("userId", response.data.id); // 수정된 부분: `id` 필드 접근
            setUserOption({ options: response.data.options }); // 수정된 부분: `options` 필드 접근
            setProfileImage(response.data.profileImage); // 프로필 이미지 설정
    ...
    //         if (userData) {
    //             console.log("응답 데이터:", response.data); // 디버깅용 로그

    //             localStorage.setItem("userId", userData.id);
    //             setUserOption({ notified: userData.options });
